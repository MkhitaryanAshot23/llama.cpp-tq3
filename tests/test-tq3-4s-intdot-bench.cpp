...
static void bench_case(ggml_backend_t backend, int k, int rows, int warmup, int iterations) {
    ...
    // Keep validation/PCIe outside the measured interval.
    float sample = 0.0f;
    ...
    std::printf("TQ3_4S intdot GPU-resident: k=%d rows=%d weights=%.3f MB iter=%d time=%.4f ms effective=%.2f GB/s sample=%g\n",
...
int main(int argc, char ** argv) {
    ...
    // This 44.56 MB matrix maps to the large FFN regime whose ~0.40 ms baseline
    // corresponds to roughly 110 GB/s effective TQ3_4S weight bandwidth.
